...
We used M-labs^[https://www.measurementlab.net/] to create a proxy for the quality of internet in a give area. To access the M-lab data we use Google Big Query and used this exact query.
...
2)  The 10% micro data from the StatsBank for data sources that need dissagregation that cannot be done on the Statsbank (https://microdata.statsghana.gov.gh/). This 10% is too big to be uploaded on Github, but can be downloaded on this microdata platform.
    ...
    2) Literacy_18_years_Internet.csv: this files contains some computed indicators on literacy and internet use of the population 18 years and older.
...
New_final <- new_df %>% 
  ...
  write.csv("Illit_eng_Lit_loc_6years_Internet.csv")
#############
...
Final <- df %>% 
  ...
  write.csv("Literacy_18_years_Internet.csv")
```
...
Rank_data <- read.csv("output_files/ranks.csv") %>%
  rename(`Internet score`=score_internet,
         ...
         Rank=rank_score,
         `Total Score` = score_final_deflated)


mydata <- census_SDG_indicators %>% 
  left_join(Rank_data , by="District")
## We segment the data into two different parts
Access_Indicator <- mydata[, c("Electricity score", "Internet score", "Education score","Total Score",  "Rank")] 
...
correlation_Matrix <- round(cor(Access_Indicator, SDG_Indicators),2)

### Converting the correlation Matrix List Into a dataframe
my_Correlation_df <- data.frame(correlation_Matrix) %>% 
  rename(`Child labour (8.7.1)`=Indicator_8.7.1_child_labour,
         `Safe water (6.1.1)`=Indicator_6.1.1_safe_water,
         `Electricity Access (7.1.1)`=Indicator_7.1.1_electricity_access,
         `Phone Ownership (5.b.1)`=Indicator_5.b.1_phone_ownership,
         `Clean Energy (7.1.2)`=Indicator_7.1.2_clean_energy_prim,
         `Safely Managed Sanitation (6.2.1)`=Indicator_6.2.1_safely_managed_sanitation,
         `Adolescent Birth Rate (3.7.2)`=Indicator_3.7.2_adolescent_birth_rate_15_19,
         `Disabled Unemployment (8.5.2)`=Indicator_8.5.2_unemployment_rate_disable,
         `Manufacturing Employment (9.2.2)`=Indicator_9.2.2_manufacturing_employment,
         `Internet Use (17.8.1)`=Indicator_17.8.1_internet_use)
...
-  Docker: for running routing server

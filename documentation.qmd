...
cor_plot <- my_Correlation_df %>%
  ...
  ggplot(aes(x = stringr::str_wrap(`SDG Indicator`, 20), y = `Access Indicator`, fill = value)) +
  geom_tile(col = "white", linewidth =1) +
  geom_text(aes(label = format(value, nsmall = 2)), size=5, family = "Azeret Mono") +
  coord_equal() +
  scale_fill_gradient2(low = "#05e1fc", mid = "white", high = "#e0224b") +
  labs(x = NULL,
       y = NULL,
       fill = "correlation") +
  theme_void()+
...
ggsave("output_files/cor_plot.png",cor_plot,  width = 12, height = 9, dpi = 300)
```
